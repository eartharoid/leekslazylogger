...
module.exports = class Functions {
	...
	static colourType (str) {
		return typeof str === 'number' ? 'eightBit' : str.includes(',') ? 'rgb' : str.startsWith('#') ? 'hex' : str.startsWith('&') ? 'CODE' : 'colours';
	} 

	static bgColourType (str) {
		return typeof str === 'number' ? 'eightBitBg' : str.includes(',') ? 'rgbBg' : str.startsWith('#') ? 'hexBg' : str.startsWith('&') ? 'CODE' : 'colours';
	} 
};