...
class Logger {
	...
	constructor(o) {
		...
		if (options.logToFile === true) {
			...
			if (options.daily === false) path = `./logs/${dtf('YYYY-MM-DD-HH-mm-ss')}.log`; // 1 per run
			else path = `./logs/${dtf('YYYY-MM-DD')}.log`; // 1 per day
	...
	createNewFile() {
		if (!fs.existsSync('./logs')) { // create logs folder if it doesn't exist
			fs.mkdirSync('./logs');
			this.verbose(this.option.keepSilent, `[${dtf(options.timestamp)} | LOGGER] No logs directory found, creating one for you`);
		}
	...
	writeLine(str) {
		if (options.daily) {
			let prev = `./logs/${dtf('YYYY-MM-DD')}.log`;
			if (path !== prev) {
				path = prev;
				this.createNewFile();
			}
