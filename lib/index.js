...
class Logger {
	...
	constructor(o) {
		...
		for (let type in this.options.types) {
			this[type] = (text, colour) => {
				...
				if (this.options.logToFile) this.writeLine(strip(format) + '\n');

				if (!leeks.supportsColour)
					return console[t.type](strip(format)); // for weirdos who don't like colours

				let fg = colour ? colour[0] : t.foreground, // foreground colour value
					bg = colour ? colour[1] : t.background, // background colour value
					fgt = 'colours', // foreground colour type
					bgt = 'colours'; // background colour type
				...
				let bgf = !bg
					? format // no background colour
					: bgt === 'colours'
						? leeks[bgt][bg](format) // standard colour name
						: leeks[bgt](bg, format); // other colour type
		...
		let custom = this.options.types.length - defaults.types.length;
		if (this.options.types.length > defaults.types.length) this.verbose(`[${this.timestamp()} | LOGGER] Initialised with ${custom} custom log ${plural('type', custom)}`);
	...
	createNewFile() {
		if (!fs.existsSync(resolve(this.options.directory))) { // create logs folder if it doesn't exist
			...
			this.verbose(`[${dtf(this.options.timestamp)} | LOGGER] No logs directory found, creating one for you`);
		}
