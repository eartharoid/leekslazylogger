...
class Logger {
	constructor(o) {
		...
		if (o.child) {
			if (!log) {
				console.error(leeks.colours.red(`[LOGGER] You can't create a child logger before calling the .multi() function (see documentation at ${pkg.homepage})`));
		...
		for (let type in data.defaultTypes) {
			...
			this[type] = async (text, colour) => {
				...
				if (this.options.logToFile) {
					...
					fs.appendFileSync(this.path, timestamp + strip(text) + '\n', (error) => {
				...
				if (leeks.supportsColour) {
					...
					let bgf = !bg ? timestamp + text : bgt === 'colours'
						? leeks[bgt][bg](timestamp + text)
						: leeks[bgt](bg, timestamp + text);

					if (!fg) console[t.log](bgf);
					else console[t.log](fgt === 'colours' ? leeks[fgt][fg](bgf) : leeks[fgt](fg, bgf));

				} else console[t.log](timestamp + strip(text)); // for weirdos who don't like colours
			};
		}
		...
		for (let type in this.custom) {
			...
			this[type] = async (text, colour) => {
				let t = this.custom[type],
					title = t.title ? ` | ${t.title.toUpperCase()}` : '',
					timestamp = `[${this.stamp() + title}] `,
					prefix = t.prefix ? `[${t.prefix.toUpperCase()}] ` : '';
	...
	multi(obj) {
		if (!obj || typeof obj !== 'object') {
			console.log(leeks.colours.red(`[${this.stamp()} | LOGGER] You must pass the Logger instance when calling the .multi() function (see documentation at ${pkg.homepage})`));
